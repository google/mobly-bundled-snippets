...
public class JsonSerializer {
  ...
  public static Bundle serializeBluetoothDevice(BluetoothDevice data) {
    ...
    final String bondState = MbsEnums.BLUETOOTH_DEVICE_BOND_STATE.getString(data.getBondState());
  ...
  private Bundle serializeBleScanRecord(ScanRecord record) {
    ...
    result.putBundle("manufacturerSpecificData", serializeBleScanManufacturerSpecificData(record));
    return result;
  }
    ...
    if (record.getServiceUuids() != null) {
      for (ParcelUuid uuid : record.getServiceUuids()) {
        ...
        if (record.getServiceData(uuid) != null) {
          service.putString(
              "Data",
              new String(Base64.encode(record.getServiceData(uuid), Base64.NO_WRAP), UTF_8));
        } else {
  ...
  public static Bundle serializeBleAdvertisingSettings(AdvertiseSettings advertiseSettings) {
    ...
    result.putString("Mode", MbsEnums.BLE_ADVERTISE_MODE.getString(advertiseSettings.getMode()));
  ...
  public static Bundle serializeBluetoothGattCharacteristic(
    ...
    result.putString(
        "Permission", MbsEnums.BLE_PERMISSION_TYPE.getString(characteristic.getPermissions()));
    return result;
  }
}
