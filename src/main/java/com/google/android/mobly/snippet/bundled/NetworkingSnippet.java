...
public class NetworkingSnippet implements Snippet {
  ...
  public String networkHttpDownload(String url)
    ...
    try {
      ...
      Log.d(String.format(Locale.US, "networkHttpDownload download of %s with id %d", url, mReqid));
      if (!Utils.waitUntil(() -> mIsDownloadComplete, 30)) {
        Log.d(String.format(Locale.US, "networkHttpDownload timed out waiting for completion"));
    ...
    if (resp != null) {
      ...
      Log.d(String.format(Locale.US, "networkHttpDownload Failed to download %s", uri.toString()));
