...
public final class Utils {
   ...
   * desired state.
   *
   * <p>If the predicate function throws an exception and interrupts the waiting, the exception will
   * be wrapped in an {@link RuntimeException}.
   *
   ...
   * callback id. Commonly used to make async calls synchronous, see {@link SmsSnippet#waitForSms()}
   * waitForSms} for example usage.
   *
   ...
   * @throws Throwable if interrupted while polling for event completion. Throws TimeoutException if
   *     no snippet event is received.
   */
   ...
   *     locate a suitable method to call. Subtypes, primitive types, boxed types, and {@code null}
  ...
  public static void adaptShellPermissionIfRequired(Context context) throws Throwable {
    if (Build.VERSION.SDK_INT >= 29) {
      Log.d(
          "Elevating permission require to enable support for privileged operation in Android Q+");
