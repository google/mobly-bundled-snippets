...
public class WifiManagerSnippet implements Snippet {
  ...
  public void wifiEnable() throws InterruptedException, WifiManagerSnippetException {
    ...
    if (!Utils.waitUntil(
        ...
        TIMEOUT_TOGGLE_STATE)) {
      throw new WifiManagerSnippetException(
          String.format("Failed to enable Wi-Fi after %ss, timeout!", TIMEOUT_TOGGLE_STATE));
    }
  }
  ...
  public void wifiDisable() throws InterruptedException, WifiManagerSnippetException {
    ...
    if (!Utils.waitUntil(
        ...
        TIMEOUT_TOGGLE_STATE)) {
      throw new WifiManagerSnippetException(
          String.format("Failed to disable Wi-Fi after %ss, timeout!", TIMEOUT_TOGGLE_STATE));
    }
  }
  ...
  @Rpc(
      description = "Get Wi-Fi scan results, which is a list of serialized WifiScanResult objects.")
  ...
  public JSONArray wifiScanAndGetResults()
    ...
    mContext.registerReceiver(
        new WifiScanReceiver(), new IntentFilter(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION));
    wifiStartScan();
    mIsScanResultAvailable = false;
    if (!Utils.waitUntil(() -> mIsScanResultAvailable, 2 * 60)) {
      throw new WifiManagerSnippetException("Failed to get scan results after 2min, timeout!");
    }
  ...
  public void wifiConnect(JSONObject wifiNetworkConfig)
    ...
    if (!mWifiManager.enableNetwork(networkId, true)) {
      throw new WifiManagerSnippetException("Failed to enable Wi-Fi network of ID: " + networkId);
    }
  ...
  @Rpc(
      description =
          "Check whether this device supports 5 GHz band Wi-Fi. " + "Turn on Wi-Fi before calling.")
  ...
  public void wifiEnableSoftAp(@Nullable JSONObject configuration) throws Throwable {
    ...
    if (!(boolean)
        Utils.invokeByReflection(mWifiManager, "setWifiApEnabled", wifiConfiguration, true)) {
